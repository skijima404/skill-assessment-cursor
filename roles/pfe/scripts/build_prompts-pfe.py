...
DEFAULT_TEMPLATES = [
    ("01_intro", Path("roles/pfe/prompts") / "01_initialize_template-pfe.md"),
    ("03_roleplay", Path("roles/pfe/prompts") / "03_roleplay_template.md"),
...
def main():
    ...
    parser.add_argument("--role", default="pfe")
    parser.add_argument("--variant", default="default")
    parser.add_argument("--config", default="roles/pfe/config/pfe.yaml")
    ...
    if "product" in fill_map:
        product_name = Path(fill_map["product"]["file"]).stem
    elif "our_pfe" in fill_map:
        product_name = Path(fill_map["our_pfe"]["file"]).stem
    else:
